...
const initial = {
  ...
  priorities: 'Flag delivery risks early. Propose at least one practical workflow improvement during the month.',
  support: 'Weekly alignment with Fazle on priorities, blockers, and decisions that need escalation.',
  summary: 'Strong overall contribution in the September baseline, with proactiveness and earlier risk identification as the main development focus.',
  reflection: '',
  meeting: false,
};

type Draft = typeof initial;
const storageKey = 'blinto-ifrat-september-2026-baseline-v1';
...
export function MonthlyReview() {
  ...
  function finalText() {
    return [
      ...
      'Status: ' + status,
      'Score: ' + total.toFixed(1) + '/100 — ' + band,
      '',
      '1. Delivery & Reliability: ' + reliability.toFixed(1) + '/10',
      ...
      'Needs improvement: ' + draft.improvements,
      ...
      '',
      'Baseline note: September is being used to validate the review system before the formal October–December cycle.',
    ].join('\n');
  }
  ...
  const completeReady = draft.reflection.trim() && draft.meeting;
  ...
  return (
    <div className="shell review-lab">
      <header className="review-lab-header">
        ...
        <p className="page-subtitle">September is a trial month to validate the values, scoring, feedback, and workflow before the formal October–December performance cycle.</p>
        <div className="hero-actions">
          ...
          <Link className="button" href="/team/ifrat">Back to Performance Card</Link>
        </div>
      </header>
      ...
      <div className="review-lab-layout">
        <div className="review-lab-main">
          <section className="panel">
            <h2>1. September baseline values</h2>
          ...
          <section className="panel">
            <h2>2. KPI values vs benchmark</h2>
          ...
          <section className="panel">
            ...
            ] as const).map(([field, label]) => <label className="review-input" key={field}>{label}<textarea rows={3} value={draft[field]} onChange={(e) => update(field, e.target.value)} /></label>)}
          </section>

          <section className="panel">
            <h2>4. Employee reflection &amp; 1:1</h2>
            <label className="review-input">Ifrat’s reflection<textarea rows={5} value={draft.reflection} onChange={(e) => update('reflection', e.target.value)} placeholder="What went well? What was challenging? What did I learn? What support do I need?" /></label>
        ...
        <aside className="panel review-summary">
          <span className="card-kicker">September baseline score</span>
          <p className="review-total">{total.toFixed(1)}<small>/100</small></p>
          <strong>{band}</strong>
          <p>Status: <strong>{status}</strong></p>
          <ul className="review-checklist">
            <li>Performance Card values: visible</li>
            <li>Manager feedback: prefilled baseline</li>
