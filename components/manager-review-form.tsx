...
export function ManagerReviewForm({ employeeSlug, monthKey, initial, canEdit, clickUpScore }: Props) {
  ...
  const scoreInput=(label:string,key:'growthScore'|'roleExcellenceScore')=><label className="review-input">{label}<input type="number" min="0" max="10" step="0.5" disabled={!canEdit} value={review[key]??''} onChange={e=>set(key,e.target.value===''?null:Number(e.target.value))}/><small>/10</small></label>;
  return <section className="panel">
    <h2>Manager review</h2>
    <p>Manager-owned monthly assessment. ClickUp evidence is read-only.</p>
